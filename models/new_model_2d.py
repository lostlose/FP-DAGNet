...
class newFeature(nn.Module):
    def __init__(self, frame_rate, network_arch, cell_arch, cell=Cell, args=None):
        ...
        for i in range(len(network_arch) - 1, -1, -1):
            if network_arch[i] == 1 and self.out_index[0] == -1:
                self.out_index[0] = i
                continue
            if network_arch[i] == 2 and self.out_index[1] == -1:
                self.out_index[1] = i
                continue
            if network_arch[i] == 3 and self.out_index[2] == -1:
                self.out_index[2] = i
                continue
        self.stride = []
        ...
        network_arch = network_layer_to_space(network_arch)
